...
1. Write the JSON schema describing your resource, [snowflake-database-grant.json](./snowflake-database-grant.json)
2. Implement your resource handlers in [handlers.ts](./snowflake-database-grant/handlers.ts)

> Don't modify [models.ts](./snowflake-database-grant/models.ts) by hand, any modifications will be overwritten when the `generate` or `package` commands are run.
