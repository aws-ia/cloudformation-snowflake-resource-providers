...
1. Write the JSON schema describing your resource, [snowflake-role-grant.json](./snowflake-role-grant.json)
2. Implement your resource handlers in [handlers.ts](./snowflake-role-grant/handlers.ts)

> Don't modify [models.ts](./snowflake-role-grant/models.ts) by hand, any modifications will be overwritten when the `generate` or `package` commands are run.
